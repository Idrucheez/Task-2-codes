...
#define TEETH                   20
#define OVERSPEED_THRESHOLD     800
#define FAULT_TIMEOUT           2
#define LOW_BATT_THRESHOLD      600
#define NUM_ADC_SAMPLES         16

// System States
#define STATE_NORMAL            0
#define STATE_OVERSPEED         1
#define STATE_SENSOR_FAULT      2
...
volatile uint32_t speed_x10 = 0;

// Fault detection
volatile uint32_t no_pulse_seconds = 0;
volatile uint32_t system_state = STATE_NORMAL;

// Display
volatile uint32_t update_flag = 0;
volatile uint32_t display_page = 0;
volatile uint32_t tick_count = 0;

// ADC
volatile uint32_t batt_voltage_hundredths = 0;
volatile uint32_t adc_raw_val = 0;

// Button / RPM selection
const uint32_t test_rpms[] = {10, 500, 3000, 0};
volatile uint32_t rpm_index = 0;
volatile uint32_t target_rpm = 10;
volatile uint32_t btn_cooldown = 0;
volatile uint32_t pulse_gen_active = 1;
...
void FormatSpeed(uint32_t spd_x10, char *buf) {
    uint32_t ipart = spd_x10 / 10;
    uint32_t fpart = spd_x10 % 10;
    char n[5];
    uint_to_str(ipart, n, 3);
    buf[0] = n[0]; buf[1] = n[1]; buf[2] = n[2];
    buf[3] = '.'; buf[4] = '0' + fpart; buf[5] = '\0';
}
...
// TIM3: Pulse generator
void TIM3_SetRPM(uint32_t rpm) {
    uint32_t pf_x10, arr_val;
    ...
    if (pf_x10 < 1000) {
        TIM3_PSC = 159;
        arr_val = (1000000 / pf_x10) - 1;
    } else {
        TIM3_PSC = 15;
        arr_val = (10000000 / pf_x10) - 1;
    }
...
void TIM2_IRQHandler(void) {
    ...
    // Compute RPM and speed
    measured_rpm = (measured_freq * 60) / TEETH;
    speed_x10 = (measured_rpm * 1131) / 1000;
    
    // Fail-safe: Track consecutive seconds with no pulses
    if (measured_freq == 0) {
        no_pulse_seconds++;
    } else {
        no_pulse_seconds = 0;
    ...
    // State Machine: Determine system state
    if (no_pulse_seconds >= FAULT_TIMEOUT) {
        system_state = STATE_SENSOR_FAULT;
    } else if (speed_x10 > OVERSPEED_THRESHOLD) {
        system_state = STATE_OVERSPEED;
    } else {
        system_state = STATE_NORMAL;
    }
    
    // Tick counter for display page switching
    tick_count++;
    if (tick_count % 5 == 0) {
...
// Button check (PA0): cycles 10 -> 500 -> 3000 -> 0
...
void UpdateDisplay(void) {
    ...
    if (system_state == STATE_SENSOR_FAULT) {
        OLED_DrawString(4, 0, "SENSOR FAULT");
        ...
        strcpy(line1, "BATT= ");
        FormatVoltage(batt_voltage_hundredths, voltBuf);
        strcat(line1, voltBuf);
        OLED_DrawString(4, 0, line1);
        
        switch (system_state) {
            case STATE_NORMAL:
                OLED_DrawString(4, 2, "STATUS= NORMAL");
                break;
            case STATE_OVERSPEED:
                OLED_DrawString(4, 2, "STATUS= OVERSPD");
                break;
        }
    }
...
int main(void) {
    ...
    // Boot screen
    OLED_ClearBuffer();
    OLED_DrawString(10, 0, "DBW MONITOR");
    OLED_DrawString(10, 2, "INITIALIZING..");
    OLED_UpdateDisplay();
    delay(2000000);
    ...
    while (1) {
        ...
        if (update_flag) {
            ...
            raw = ADC1_ReadAvg(NUM_ADC_SAMPLES);
            adc_raw_val = raw;
            batt_voltage_hundredths = (raw * 890 + 2047) / 4095;
